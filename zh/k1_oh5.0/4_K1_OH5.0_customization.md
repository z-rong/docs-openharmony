...
3. 修改`system/etc/app`目录下的系统应用配置，更改里面所有的包含launcher中`app_signature`字段，需要保持和替换的hap指纹一致

4. 系统重启后，替换安装新构建的 Launcher 即可生效。
...
#### 2.5.2 修改系统配置文件开机不启动systemUI

1. 在 `system/etc/app/install_list_capability.json` 文件中修改应用启动相关的配置，将`keepAlive`值改为`false`，修改如下：
...
2. 由于systemUI应用包含锁屏相关服务，同样也需要在 `/system/etc/init/foundation.cfg` 文件中去除开机时对锁屏应用状态的检测，如下：
...
副作用：systemui的功能包含快捷键（如蓝牙，wifi，设置）和遥控器键值的监听，熄屏操作的监听，悬浮球的监听，如果去除systemui，这些功能都无法使用，如果有相关功能，需要移植到自己的应用中
...
1. 确保 Camera　底层驱动正常，保证 `cam-test` 的 demo 正常出图。如需添加新型号 Sensor，参考 [K1_OH5.0 MIPI 摄像头适配指南](12_K1_OH5.0_mipi_sensor.md)
