  ...
  - 升级组件配置文件夹 `updater_config`，该文件夹内部包括：
    ...
    <package>
        <head name="Component header information">
            <info fileVersion="02" prdID="123456" softVersion="OpenHarmony 6.1.0.31" date="2025-02-13" time="14:30">head info</info>
        </head>
...
- 更新 `target_package` 目录中的文件
  ...
  <package>
      <head name="Component header information">
          <info fileVersion="02" prdID="123456" softVersion="OpenHarmony 6.1.0.32" date="2025-02-14" time="14:30">head info</info>
      </head>
      <group name = "Component information">
...
在 OH6.1 系统上，遇到生成的二进制 `updater_binary` 无法执行，执行会触发 `signal11`。经过和 OH4.0 对比，发现 OH6.1 并未将 `updater_binary` 定义为 `executable`，怀疑这导致了运行异常。
