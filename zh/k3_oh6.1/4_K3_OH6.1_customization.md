...
#### 2.5.2 修改系统配置文件不启动systemUI

1. 在 `system/etc/app/install_list_capability.json` 文件中修改应用启动相关的配置，将`keepAlive`值改为`false`如下：
...
注意：由于 OH6.1 版本 hid ddk 的功能实现尚未完全，可通过直接读写 hidraw 节点实现通信。
